...
async def webhook(request: Request):
    ...
    try:
        ...
        answer = (response.output_text or "").strip() or "Не удалось получить ответ."
        await send_long_message(bot, chat_id, answer)
    except Exception:
        logger.exception("OpenAI/Telegram error")
        await bot.send_message(chat_id=chat_id, text="Ошибка при обработке запроса. Проверь ключи и логи Vercel.")
